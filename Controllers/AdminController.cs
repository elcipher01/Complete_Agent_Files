    ...
    ORDER BY al.AvailableSlotCount DESC,  -- most available slots first (least busy)
        ...
        public async Task<IActionResult> QueueAssign([FromBody] AssignSessionRequest model)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                // Prevent assignment unless the chosen slot is truly available right now.
                using (var cmd = new SqlCommand(
                    $@"SELECT TOP 1 AgentStatus, ChatStatus
                       FROM {slotTable}
                       WHERE AgentID = @AgentID", connection))
                {
                    ...
                    using var reader = await cmd.ExecuteReaderAsync();
                    if (!await reader.ReadAsync())
                        return Json(new { success = false, message = "Agent slot was not found." });

                    var currentAgentStatus = reader["AgentStatus"]?.ToString() ?? "";
                    var currentChatStatus = reader["ChatStatus"]?.ToString() ?? "";
                    var agentOk = string.Equals(currentAgentStatus, "Available", StringComparison.OrdinalIgnoreCase);
                    var chatOk = string.Equals(currentChatStatus, "Available", StringComparison.OrdinalIgnoreCase);
                    ...
                    if (!agentOk || !chatOk)
                    {
                        ...
                        {
                            success = false,
                            message = $"Agent is not assignable (AgentStatus={currentAgentStatus}, ChatStatus={currentChatStatus})."
                        });
                    }
                }
